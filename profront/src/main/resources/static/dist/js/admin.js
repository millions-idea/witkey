...
function mkDialog(u, c, t, w, s, p, px, py) {
    ...
    var body_scrollTop = body.scrollTop ? body.scrollTop : document.body.scrollTop;
    var bw = parseInt((bsw < cw) ? cw : bsw);
    var bh = parseInt((bsh < ch) ? ch : bsh);
    if (c.indexOf('DP_image') != -1) {//手机网站缩略图适应屏幕
        var max_width = cw - 50;
        c = $(c).attr('style', 'max-width: ' + max_width + 'px').prop("outerHTML");
    }
    //是否上传对话框
    var isupload = 0;
    if (c.indexOf('/upload.php') > 0) {
        isupload = 1
    }
    if (!s && !isupload) {
    ...
    var st = py ? py : body_scrollTop + parseInt(ch / 2) - 100;
    ...
    if (!isupload) {
        $('#Dtop').show(1, function () {
            st = py ? py : body_scrollTop + parseInt(ch / 2) - parseInt($('#Dtop').height() / 2);
            $('#Dtop').animate({top: st + 'px'}, 1, function () {
                if (c.indexOf('DP_image') != -1) {
                    ...
                    $('.DP_image').load(function () {
                        ...
                        if (_sl < 10) _sl = 10;
                        var _st = py ? py : body_scrollTop + parseInt((ch - $('.DP_image').height()) / 2);
                        if (_st < 10) _st = 10;
...
function Dvname(m, w, h, o, s, i) {
    ...
    var i = i ? i : 'thumb';
    var c = '<iframe name="UploadThumb" style="display:none;" src=""></iframe>';
    c += '<form method="post" target="UploadThumb" enctype="multipart/form-data" action="/upload.php" onsubmit="return isUP(\'upthumb\');"><input type="hidden" name="moduleid" value="' + m + '"/><input type="hidden" name="from" value="thumb"/><input type="hidden" name="type" value="vname"/><input type="hidden" name="old" value="' + o + '"/><input type="hidden" name="fid" value="' + i + '"/><input type="hidden" name="type" value="vname"/><table cellpadding="3"><tr><td><input id="remote_0" type="radio" name="isremote" value="0" checked onclick="ReLo(0, \'upthumb\');"/><label for="remote_0"> ' + L['up_local'] + '</label>&nbsp;&nbsp;&nbsp;<input id="remote_1" type="radio" name="isremote" value="1" onclick="ReLo(1, \'upthumb\');"/><label for="remote_1"> ' + L['up_remote'] + '</label></td></tr><tr id="remote_url" style="display:none;"><td><input id="remote" type="text" size="30" name="remote" value="http://" onclick="if(this.value==\'http://\')this.value=\'\';"/></td></tr><tr id="local_url"><td><input id="upthumb" type="file" size="20" name="upthumb" onchange="if(isImg(\'upthumb\')){this.form.submit();Dd(\'Dsubmit\').disabled=true;Dd(\'Dsubmit\').value=\'' + L['uploading'] + '\';}"/></td></tr><tr style="display:' + s + '"><td>' + L['width'] + ' <input type="text" size="3" name="width" value="' + w + '"/> px &nbsp;&nbsp;&nbsp;' + L['height'] + ' <input type="text" size="3" name="height" value="' + h + '"/> px </td></tr><tr><td><input type="submit" class="btn" value="' + L['upload'] + '" id="Dsubmit"/>&nbsp;&nbsp;<input type="button" class="btn" value="' + L['cancel'] + '" onclick="cDialog();"/></td></tr></table></form>';
    mkDialog('', c, L['upload_img'], 250);
}

///文件上传的
function Dalbum(f, m, w, h, o, s, t) {
    var s = s ? 'none' : '';
    var c = '<iframe name="UploadAlbum" style="display:none" src=""></iframe>';
    c += '<form method="post" target="UploadAlbum" enctype="multipart/form-data" ' +
        'action="/fileup/fileUpload" onsubmit="return isUP(\'upalbum\');">' +
        '<input type="hidden" name="fid" value="' + f + '"/>' +
        '<input type="hidden" name="moduleid" value="' + m + '"/>' +
        '<input type="hidden" name="from" value="album"/>' +
        '<input type="hidden" name="old" value="' + o + '"/>' +
        '<input type="hidden" name="type" value="' + t + '"/>' +
        '<table cellpadding="3"><tr><td>' +
        '<input id="remote_0" type="radio" name="isremote" value="0" checked onclick="ReLo(0, \'upalbum\');"/>' +
        '<label for="remote_0"> ' + L['up_local'] + '</label>&nbsp;&nbsp;&nbsp;' +
        '<input id="remote_1" type="radio" name="isremote" value="1" onclick="ReLo(1, \'upalbum\');"/>' +
        '<label for="remote_1"> ' + L['up_remote'] + '</label></td></tr>' +
        '<tr id="remote_url" style="display:none;"><td>' +
        '<input id="remote" type="text" size="30" name="remote" value="http://" ' +
        'onclick="if(this.value==\'http://\')this.value=\'\';"/></td></tr>' +
        '<tr id="local_url"><td>' +
        '<input id="upalbum" type="file" size="20" name="upalbum" onchange="if(isImg(\'upalbum\'))' +
        '{this.form.submit();Dd(\'Dsubmit\').disabled=true;' +
        'Dd(\'Dsubmit\').value=\'' + L['uploading'] + '\';}"/></td></tr><tr style="display:' + s + '">' +
        '<td>' + L['width'] + ' <input type="text" size="3" name="width" value="' + w + '"/> px &nbsp;&nbsp;&nbsp;' + L['height'] + ' ' +
        '<input type="text" size="3" name="height" value="' + h + '"/> px </td></tr><tr><td>' +
        '<input type="submit" class="btn" value="' + L['upload'] + '" id="Dsubmit"/>&nbsp;&nbsp;' +
        '<input type="button" class="btn" value="' + L['cancel'] + '" onclick="cDialog();"/></td></tr></table></form>';
    mkDialog('', c, L['upload_img'], 250);
}
...
function getAlbum(v, i) {
    Dd('thumb' + i).value = v;
    Dd('showthumb' + i).src = v;
}

function delAlbum(i, s) {
    Dd('thumb' + i).value = '';
    Dd('showthumb' + i).src = SKPath + 'image/' + s + 'pic.png';
}

function selAlbum(i) {
    Dwidget(DTPath + 'api/select.php?from=album&fid=' + i, L['choose_img'], 730, 300)
}
...
function isImg(i, e) {
    var v = Dd(i).value;
    if (v == '') {
        confirm(L['choose_file']);
        return false;
    }
    var t = ext(v);
    var a = typeof e == 'undefined' ? 'jpg|gif|png|jpeg|bmp' : e;
    if (a.length > 2 && a.indexOf(t) == -1) {
        confirm(L['allow'] + a);
        return false;
    }
    return true;
...
function check_box(f, t) {
    var t = t ? true : false;
    var box = Dd(f).getElementsByTagName('input');
...
function schcate(i) {
    Dh('catesch');
    var name = prompt(L['type_category'], '');
    if (name) makeRequest('moduleid=' + i + '&action=schcate&name=' + name, AJPath, '_schcate');
}

function _schcate() {
    if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
        Ds('catesch');
        Dd('catesch').innerHTML = xmlHttp.responseText ? '<strong>' + L['related_found'] + '</strong><br/>' + xmlHttp.responseText : '<span class="f_red">' + L['related_not_found'] + '</span>';
    }
}

function reccate(i, o) {
    if (Dd(o).value.length > 1) {
        Dh('catesch');
        makeRequest('moduleid=' + i + '&action=reccate&name=' + Dd(o).value, AJPath, '_reccate');
    }
}
...
function ckpath(m, i) {
    if (Dd('filepath').value.length > 4) {
        makeRequest('moduleid=' + m + '&action=ckpath&itemid=' + i + '&path=' + Dd('filepath').value, AJPath, '_ckpath');
    } else {
        alert(L['type_valid_filepath']);
        Dd('filepath').focus();
    }
}

function _ckpath() {
    if (xmlHttp.readyState == 4 && xmlHttp.status == 200) Dd('dfilepath').innerHTML = xmlHttp.responseText;
}

function tpl_edit(f, d, i) {
    var v = Dd('destoon_template_' + i).firstChild.value;
    var n = v ? v : f;
    Dwidget('?file=template&action=edit&fileid=' + n + '&dir=' + d, L['tpl_edit']);
}

function tpl_add(f, d) {
    Dwidget('?file=template&action=add&type=' + f + '&dir=' + d, L['tpl_add']);
}

function _ip(i) {
    mkDialog('', '<iframe src="?file=ip&js=1&ip=' + i + '" width="180" height="70" border="0" vspace="0" hspace="0" marginwidth="0" marginheight="0" framespacing="0" frameborder="0" scrolling="no"></iframe>', 'IP:' + i, 200, 0, 0);
}

function _mobile(i) {
    mkDialog('', '<iframe src="?file=mobile&js=1&mobile=' + i + '" width="180" height="70" border="0" vspace="0" hspace="0" marginwidth="0" marginheight="0" framespacing="0" frameborder="0" scrolling="no"></iframe>', i, 200, 0, 0);
}

function _user(n, f) {
    var f = f ? f : 'username';
    Dwidget('?moduleid=2&action=show&dialog=1&' + f + '=' + n, lang(L['dialog_user'], [n]));
}

function perc(v, t, w) {
    var p = t == 0 ? 0 : Math.round(100 * v / t);
    document.write('<div class="perc" style="width:' + w + '" title="' + p + '%"><div style="width:' + p + '%;">&nbsp;</div></div>');
}
...
function _islink() {
    if (Dd('islink').checked) {
        ...
        Dh('basic');
        Df('linkurl');
        if (Dd('linkurl').value == '') Dd('linkurl').value = 'http://';
    } else {
        Dh('link');
        Ds('basic');
    }
...
function _preview(s, t, html) {
    html = html || '';
    var t = t ? true : false;
    if (s) {
        if (t) {
            var p = s.lastIndexOf('.thumb.');
            if (p != -1) s = s.substring(0, p);
        }
        if (s.indexOf('.thumb.') != -1) {
            s = s.replace('.thumb.', '.middle.');
        }
        mkDialog('', '<img src="' + s + '" class="DP_image" onclick="cDialog();" ' + html + ' />', L['preview_img']);
    } else {
        Dtip(L['empty_img']);
    }
}
...
function select_item(m, f) {
    f = f ? f : '';
    Dwidget(DTPath + 'api/select.php?mid=' + m + '&action=item&from=' + f, L['choose_item'], 755, 300);
}

function Menuon(i) {
    try {
        Dd('Tab' + i).className = 'tab_on';
    } catch (e) {
    }
}
